// Reads a chat answer aloud in the store's ElevenLabs voice (Adam's clone,
// the shared "Porchlight" voice). GET ?text= or POST { text } -> audio/mpeg.
...
// Text-to-speech mangles abbreviations ("St", "OH"), so expand the ones our
// content actually uses before synthesis. Word-boundary matches only.
// Time rules run BEFORE the phone/ZIP digit rules on purpose.
export const expandForSpeech = (t) =>
  t
    ...
    // Phone numbers and ZIP codes read digit by digit, never as "three hundred
    // thirty". Commas give natural pauses.
